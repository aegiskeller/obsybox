...
bool isSystemResponsive() {
  ...
  if (WiFi.getMode() != WIFI_OFF) {
    // Test WiFi AP functionality (safe check)
    IPAddress ip = WiFi.softAPIP();
    if (ip == IPAddress(0, 0, 0, 0)) {
      return false;  // WiFi AP has issues
    }
  }
...
void setup() {
  Serial.begin(115200);
  Serial.println("Starting ESP32-CAM AP...");
...
void loop() {
  ...
  if (watchdogActive && currentTime - lastWatchdogCheck > 5000) {
    ...
    if (loopCounter % 6 == 0) {
      Serial.println("System Health: " + String(systemHealthy ? "OK" : "DEGRADED"));
