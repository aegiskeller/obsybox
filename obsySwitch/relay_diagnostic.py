...
def test_relay_switching():
    ...
    try:
        ...
        # Get initial status
        print("1£ Getting initial relay status...")
        ...
        print(f"\n2£ Testing Relay 1 (Pin 2) - Listen for click sounds!")
        print(f"  Make sure you can hear the relay module...")
        ...
        for step, command in test_cycles:
            ...
            if user_input == 'q':
                ...
                print(f"   Relay is switching!")
            elif user_input == 'n':
                print(f"   No sound detected")
            else:
                print(f"    Please answer y/n/q")
            
            time.sleep(1)
        
        print(f"\n3£ Testing rapid switching (should hear multiple clicks)...")
        for i in range(5):
        ...
        print(f"  Relay is working correctly!")
        print(f"  Wiring is good")
        print(f"  Arduino code is working")
        
        print(f"\nIf you heard NO clicks:")
        print(f"  Check these items:")
        print(f" 1. Relay module power: VCC to Arduino 5V, GND to Arduino GND")
        print(f" 2. Signal wire: IN1 connected to Arduino pin 2")
        print(f" 3. Relay module type: Try changing active HIGH/LOW setting")
        print(f" 4. Power supply: Some relay modules need external 5V power")
        print(f" 5. Relay module LED: Should light up when activated")
        
        print(f"\n NEXT STEPS:")
        print(f" - If relay clicks: System is working correctly!")
        print(f" - If no clicks: Check wiring and power")
        print(f" - If LED lights but no click: Relay may need external power")
