...
def relay_sequence_demo():
    ...
    print(f"ObsyBox Relay Sequence Demo")
    print("=" * 50)
    ...
    if not controller.connect():
        print(f"Arduino not found - check USB connection")
        return False
    ...
    try:
        ...
        print(f"SEQUENCE 1: Turning ON relays 1234")
        print("-" * 40)
        ...
        print(f"Uptime: {final_info.uptime/1000:.1f} seconds")
        ...
        for i in range(4):
            controller.set_switch(i, False)
            time.sleep(0.1)
        print(f"All relays turned off safely")
        
    except Exception as e:
        print(f"\n Error during demo: {e}")
        print(f"Turning off all relays for safety...")
        for i in range(4):
            try:
                controller.set_switch(i, False)
            except:
...
if __name__ == "__main__":
    print(f"Starting Relay Sequence Demo...")
    print(f"  Make sure you can hear the relay clicks!")
